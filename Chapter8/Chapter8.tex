...
本章介绍的\alert{动态内存分配}技术的提出就是了为了解决这个问题
\end{greenblock}
...
\begin{block}{动态对象}
\alert{由程序员创建并负责回收}的对象，在动态内存（也称为自由存储区或堆）中被创建
\begin{itemize}
  \item 动态存储周期
  \item 在全局数据区被分配存储空间
\end{itemize}
...
\uncover<3->{
释放一块{\color{red}非\texttt{new}分配的内存、同一内存多次释放或者使用一个已经释放的内存}，其行为都是未定义的：}
...
\begin{block}{空悬指针}
对于一个指向动态内存的指针，在\texttt{delete}之后会依然保存已经释放的内存地址，此时的指针也称为空悬指针
\end{block}
...
\begin{enumerate}
  \item \texttt{unique\_ptr}独占所指向的对象
  \item \texttt{shared\_ptr}允许多个指针指向一个对象
...
\begin{blueblock}<4->{\texttt{unique\_ptr}使用一}
...
\column{0.3\textwidth}
\begin{yellowblock}<2->{说明}
当\texttt{p2}消亡时，\texttt{p2}所指向的对象也会消亡，完成动态内存的自动释放\\
函数模板make\_unique模板参数为指针指向的类型，实参为类型构造函数参数
\end{yellowblock}
...
\texttt{unique\_ptr}~\alert{独自}拥有所指向的动态对象，也就是说只能有一个\texttt{unique\_ptr}指向给定
的对象：
...
\column{0.3\textwidth}
\begin{yellowblock}<2->{说明}
\texttt{unique\_ptr}独占所指向的对象，不支持拷贝和赋值
\end{yellowblock}
...
\begin{yellowblock}<2->{说明}
\begin{itemize}
  \item \texttt{release}将\texttt{p1}置为\texttt{nullptr}并返回\texttt{p1}原来的指针
  \item p1将其内存所有权转移给p2
\end{itemize}
...
\begin{yellowblock}<3->{说明}
\begin{itemize}
  \item \texttt{reset}释放\texttt{p3}原来的动态内存，并指向\texttt{p2}释放出来的内存
\end{itemize}
...
同\texttt{unique\_ptr}，必须使用直接初始化的形式来初始化一个\texttt{shared\_ptr}：
...
\column{0.65\textwidth}
\begin{blueblock}<2->{\texttt{shared\_ptr}初始化一}
...
\column{0.65\textwidth}
\begin{blueblock}<4->{\texttt{shared\_ptr}初始化二}
\vspace{-1.5mm}\begin{lstlisting}[moreemph={shared\_ptr}]
...
与\texttt{unique\_ptr}不同，\texttt{shared\_ptr}允许复制或赋值：
...
\end{yellowblock}
\begin{yellowblock}<3->{说明}
对一个\texttt{shared\_ptr}类型对象进行赋值时，赋值操作符将左操作数所指对象的引用计数减\texttt{1}（如果引用计数减至为\texttt{0}，则消亡其指向的对象），并将右操作数所指对象的引用计数加\texttt{1}
\end{yellowblock}
...
\column{0.3\textwidth}
\begin{yellowblock}<4->{说明}
\texttt{lock}函数检查指向的对象是否存在，如果对象存在则返回一个可用的\texttt{shared\_ptr}，否则返
...
\column{0.3\textwidth}
\begin{yellowblock}<2->{说明}
\texttt{new}为动态数组分配指定大小的内存，并返回第一个元素的地址
\end{yellowblock}
\begin{yellowblock}<3->{说明}
...
当定义一个类时，编译器将为我们自动合成\alert{默认}的复制构造函数、赋值运算符和析构函数。\\
~\\
...
\column{0.65\textwidth}
\begin{blueblock}{\texttt{MyStr}类辅助函数定义}
\begin{lstlisting}[moreemph={MyStr}]
...
\end{lstlisting}
\end{blueblock}
\begin{blueblock}<2->{\texttt{MyStr}类对象复制}
\begin{lstlisting}[moreemph={MyStr}]
{
...
\column{0.27\textwidth}
\begin{yellowblock}<2->{说明}
相比复制构造函数，\alert{直接接管}给临时对象分配的动态内存。没有分配新的动态内存，也没有对动态内存数据的复制。
\end{yellowblock}
\vspace{-2mm}
\begin{greenblock}<3->{问题}
移动构造中若没有置空临时对象\texttt{rhs}的指针成员会怎样？
\end{greenblock}
\vspace{-2mm}
...
\only<4->{\includegraphics[scale=0.43]{array_2}}
\end{center}
...
尾插操作将新结点插入到链表的表尾：
\only<1>{\includegraphics[width=\textwidth]{push_back1}}
\only<2>{\includegraphics[width=\textwidth]{push_back2}}
\only<3->{\includegraphics[width=\textwidth]{push_back3}}
...
\column{0.65\textwidth}
\begin{blueblock}<2->{\texttt{push\_back}函数定义}
...
\only<1>{\includegraphics[width=\textwidth]{insert1}}
\only<2>{\includegraphics[width=\textwidth]{insert2}}
\only<3->{\includegraphics[width=\textwidth]{insert3}}
\vspace{-4mm}
...
\column{0.6\textwidth}
\begin{blueblock}<2->{\texttt{insert}函数定义}
\begin{lstlisting}[moreemph={SList,T,Node}]
template<typename T>
...
\begin{center}\includegraphics[width=\textwidth]{erase}\end{center}
...
回顾类模板 Node的定义，如果使用默认的复制与赋值操作会有什么后果？
...
\column{0.65\textwidth}
\begin{blueblock}{\texttt{Node}类模板部分定义}
\begin{lstlisting}[moreemph={Node,T}]
template<typename T>
class Node{
    T m_data; //数据域
...
\column{0.65\textwidth}
\begin{blueblock}<3->{\texttt{Node}类模板部分定义}
...
\vspace{-2mm}
\begin{center}
  \includegraphics[width=\textwidth]{linked_list}
\end{center}
链栈：\\
\vspace{-4mm}
\begin{center}
...
\only<3>{\includegraphics[width=\textwidth]{create_binary_tree_1}}
\only<4>{\includegraphics[width=\textwidth]{create_binary_tree_2}}
\only<5>{\includegraphics[width=\textwidth]{create_binary_tree_3}}
\only<6>{\includegraphics[width=\textwidth]{create_binary_tree_4}}
\only<7>{\includegraphics[width=\textwidth]{create_binary_tree_5}}
\only<8>{\includegraphics[width=\textwidth]{create_binary_tree_6}}
\only<9>{\includegraphics[width=\textwidth]{create_binary_tree_7}}
\only<10>{\includegraphics[width=\textwidth]{create_binary_tree_8}}
\only<11->{\includegraphics[width=\textwidth]{create_binary_tree_9}}
...
\uncover<3->{输出结果为：\texttt{1 3 4 6 7 8 10 13 14}}
