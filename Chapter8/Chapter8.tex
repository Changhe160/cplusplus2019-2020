...
\begin{block}{动态对象}
由程序员创建并负责回收的对象，在\alert{动态内存}（也称为自由存储区或堆）中被创建
\begin{itemize}
  \item 动态存储周期
  \item 在全局数据区被分配存储空间
\end{itemize}
...
\uncover<2->{\begin{center}\includegraphics[scale=0.7]{array}\end{center}}

\vspace{6mm}

\uncover<3->{线性链表为\alert{链式结构}，在逻辑结构上相邻的元素在物理结构上\alert{不要求}相邻：}
\uncover<3->{\begin{center}\includegraphics[scale=0.7]{linked_list}\end{center}}
...
尾插操作将新结点插入到链表的表尾：
\begin{center}\includegraphics[width=0.9\textwidth]{push_back}\end{center}
