...
inline double Fraction::value() const{
    return static_cast<double>(m_numerator) / m_denominator;//(p.27)
}
...
Fraction e = divide(b, c);
\end{lstlisting}\vspace{-1.5mm}
\onslide<1->divide函数调用过程中Fraction类构造函数调用几次？
\end{greenblock}
...
\begin{itemize}
\item 一类特殊的函数；
  \item 函数名由关键字~\textcolor{blue}{operator}\index{O!operator!operator overloading}~和需要重载的运算符共同组成；
